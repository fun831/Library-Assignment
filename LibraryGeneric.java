...
/**
 * Generic class representation of a library (a collection of library books).
 * The generic is derived from the Library class.
 * 
...
public class LibraryGeneric<Type> {
	...
	public ArrayList<LibraryBookGeneric<Type>> lookup(Type holder) {
		ArrayList<LibraryBookGeneric<Type>> booksOfHolder = new ArrayList<LibraryBookGeneric<Type>>();
		for(int i = 0; i < library.size(); i++) {
			if(library.get(i).getHolder() == holder) {
				booksOfHolder.add(library.get(i));
			}
		}
	...
	public boolean checkin(Type holder) {
		boolean anyCheckedBackIn = false;
		
		for(int i = 0; i < library.size(); i++) {
			if(library.get(i).getHolder() == holder) {
				library.get(i).checkIn();
				
				anyCheckedBackIn = true;
			}
		}
		return anyCheckedBackIn; 
	}
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month, int day,int year) {
		...
		for(int i = 0; i < library.size(); i++) {
			if(library.get(i).getDueDate() != null) {
				if(library.get(i).getDueDate().compareTo(dueDate) < 0) {
	...
	protected class OrderByAuthor implements Comparator<LibraryBookGeneric<Type>> {
		...
		/**
		 * Returns a negative value if lhs is smaller than rhs. Returns a positive
		 * value if lhs is larger than rhs. If they are equal then the tie is 
		 * broken by ordering by the title.
		 */
		@Override
		public int compare(LibraryBookGeneric<Type> lhs, LibraryBookGeneric<Type> rhs) {
			int diff = lhs.getAuthor().compareTo(rhs.getAuthor());						
